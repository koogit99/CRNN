...
#             noisy = noisy.permute(0, 1, 3, 2) ### [B, 1, F, T] => [B, 1, T, F]
#             clean = clean.permute(0, 1, 3, 2) ### [B, 1, F, T] => [B, 1, T, F]
#             enhanced = self.model(noisy)  # [B, 1, F, T] -> [B, 1, T, F]

#             ###
#             # Convert back to [B, F, T] for the loss function
#             enhanced = enhanced.squeeze(1)  # [B, 1, T, F] => [B, T, F]
#             clean = clean.squeeze(1)  # [B, 1, T, F] => [B, T, F]
#             enhanced = enhanced.permute(0, 2, 1)  # [B, T, F] => [B, F, T]
#             clean = clean.permute(0, 2, 1)  # [B, T, F] => [B, F, T]
#             ###
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for noisy, clean, n_frames_list, _ in self.train_dataloader:
            ...
            clean = clean.squeeze(1)  # [B, 1, T, F] => [B, T, F]
            enhanced = enhanced.permute(0, 2, 1)  # [B, T, F] => [B, F, T]
            print("enhanced shape before loss : ", enhanced.shape)
            clean = clean.permute(0, 2, 1)  # [B, T, F] => [B, F, T]
            print("clean shape before loss : ", clean.shape)
            ###
