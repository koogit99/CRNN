...
def collate_fn_pad(batch): # main의 로컬 범위 안에서 전역 범위로 이동시킴
        """
        Returns:
            [B, F, T (Longest)]
        """
        ...
        noisy_list = pad_sequence(noisy_list).permute(1, 0, 2)  # ([T1, F], [T2, F], ...) => [T, B, F] => [B, F, T]
        # => [B, T, F]
        clean_list = pad_sequence(clean_list).permute(1, 0, 2)  # ([T1, 1], [T2, 1], ...) => [T, B, 1] => [B, 1, T]
        # => [B, T, F]
